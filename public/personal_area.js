...
function fetch_all(){
    fetch("fetch_favourites").then(fetchResponse).then(fetchCollectionJson); 
    const user_id = "true";
    const id = "null"
    fetch(`fetch_auction/${id}/${user_id}`).then(fetchResponse).then(fetchAuctionJson); 
    fetch("fetch_myoffers").then(fetchResponse).then(fetchOffersJson);
    fetch("fetch_notifications").then(fetchResponse).then(fetchNotificationJson);
    fetch("fetch_saved_unsaved_tour").then(fetchResponse).then(fetchTourJson);
}
...
function show_auction_details(event){
    const id = encodeURIComponent(event.currentTarget.dataset.index);
    window.location.href = `info_auction/${id}`;
}
...
function fetchNotificationJson(json){
    if(json.length==0){
        ...
        function hide_all(){
            const hide = "true";
            fetch(`fetch_notifications/${hide}`).then(fetchResponse).then(fetchNotificationJson);    
        }
...
function remove_tour(event){
    ...
    fetch(`fetch_saved_unsaved_tour/${id_remove}`).then(fetchResponse).then(fetchRemoveTourJson);
}
