...
function fetch_all(){
    ...
    const mg = "magazine";
    const sp = "sponsor";
    fetch(`fetch_images/${ex}`).then(fetchResponse).then(fetchExibitionsJson);
    fetch(`fetch_images/${mg}`).then(fetchResponse).then(fetchMagazineJson);
    fetch(`fetch_images/${sp}`).then(fetchResponse).then(fetchSponsorJson);
    fetch("fetch_collection").then(fetchResponse).then(fetchCollectionJson);
}
...
function show_collection_details(event){
    const id = encodeURIComponent(event.currentTarget.dataset.index);
...
function search(event) {
    ...
    const url = `/fetch_hotel/${check_in_value}/${check_out_value}/${adults_value}/${rooms_value}`;
...
function searchArtist(event){
    ...
    fetch(`/fetch_artsy/${user_input}`).then(onResponse).then(onJsonArtist);
}
...
function onJsonArtist(json){
    if(json.message == "Artist Not Found"){
        ...
        first_param = 'null'; 
        fetch(`/fetch_artsy/${first_param}/${artist_id}`).then(onResponse).then(onJsonArtworks);
    }
}
...
function searchTour(event) {
    fetch("fetch_tour").then(onResponse).then(onTourJson);
}
...
function saveTour(event){
    ...
    fetch("fetch_save_tour", {method: 'post', body: formData, headers: {'X-CSRF-TOKEN': token}}).then(onResponse).then(onAddTourJson);
